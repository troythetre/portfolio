...
export default async function getQuestionDeadlines(req, res) {
  if (req.method === 'GET') {
    try {
      ...
      if (existingProposal.isDeleted && existingProposal.isDeleted.status) {
        res.status(400).json({ message: 'Cannot perform action on deleted proposal' });
        return;
      }
      ...
      if (existingProposal.questions[questionIndex].isDeleted && existingProposal.questions[questionIndex].isDeleted.status) {
        res.status(400).json({ message: 'Cannot perform action on deleted question' });
        return;
      }
     
      // Find the review and write deadlines
      const deadlineEntry = {
        review_deadline: existingProposal.questions[questionIndex].review_deadline || 'N/A',
        write_deadline: existingProposal.questions[questionIndex].write_deadline || 'N/A',
      };
