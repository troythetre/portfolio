...
export default function WritersList({ currentQuestion, setCurrentQuestion, setEditWriters, setWritersBody, addingWriters, setAddingWriters, teamMembers, questions, RFPID, onWriterUpdate }) {
    const [newWriter, setNewWriter] = useState("");
    const [addDate, setAddDate] = useState(false);
    const [date, setDate] = useState(null);
    const [currentWriters, setCurrentWriters] = useState(currentQuestion.members.map(member => member.teamMember_email));
    ...
    // allows us to change the selected writer
    const handleCheckboxChange = (userEmail, isChecked) => {
        if (isChecked) {
            setNewWriter(userEmail); // Only add the selected email
        } else {
            setNewWriter(''); // If unchecked, clear the selection
        }
    };
    ...
    const handleSubmit = async () => {
        if (!date) {
            alert('Please select a date'); // If no date is selected, alert the user
            return;
        } else {
            try {
                const displayName = getName(newWriter)
                let writer = { name: displayName, email: newWriter, deadline: date }
                let secWriter = { writerName: displayName, writerEmail: newWriter, writerDeadline: date }
                const response = await fetch(`${BASEURL}/api/proposal/question-addWriter`, {
                    method: "PUT",
                    credentials: "include",
                    headers: {
                ...
                if (response.ok) {
                    currentQuestion.members = [...currentQuestion.members, writer]
                    questions.find((question) => {
                        if (question.questionID === currentQuestion.questionID) {
                            question.members = currentQuestion.members
                        }
                    })
                    const updatedWriters = [...currentQuestion.writers, secWriter];


                    if (onWriterUpdate) {
                        onWriterUpdate(updatedWriters);
                    }
                    setAddDate(false)
                    setEditWriters(false)
                    console.log("Successfully added writer")
                } else {
                    console.log("Failed to add writer")
            ...
            <div className="w-full bg-zinc-700 rounded-lg p-8">
                ...
                <div className="border border-solid border-r-0 border-l-0 border-t-0 border-zinc-900 flex justify-between pb-8">
                    ...
                    <div className="flex gap-1">
                        ...
                        <p className="text-white">Add Writers</p>
                    </div>
                    ...
                    <div className={`h-[75px] overflow-y-scroll ${styles.inputOverflow}`}>
                        {
                            teamMembers && teamMembers.length > 0
                                ...
                                teamMembers.map((teamMember, index) => {
                                    const displayName = getName(teamMember.teamMember_email);
                                    return (
                                        <div key={index} className="flex items-center">
                                            <label className="flex items-center cursor-pointer">
                                                <input
                                                    className="mr-2"
                                                    type="checkbox"
                                                    checked={newWriter === teamMember.teamMember_email}
                                                    onChange={(e) => handleCheckboxChange(teamMember.teamMember_email, e.target.checked)}
                                                />
                                                <div className={`custom-checkbox ${newWriter === teamMember.teamMember_email ? 'checked' : ''}`}></div>
                                                <div>
                                                    <p className="text-white">{displayName ? displayName : "Unknown Name"}</p>
                ...
                {addDate &&
                    <div className={`h-[75px] overflow-y-scroll ${styles.inputOverflow}`}>
                        <div className="flex justify-between items-center">
                            <div>
                                <p className="text-white">{newWriter}</p>
                            </div>
                ...
                {addingWriters &&
                    <div className="flex justify-center pt-8">
                        <button
                            ...
                            onClick={() => {
                                setAddDate(true)
                                setAddingWriters(false)
                            }}
                            disabled={!(teamMembers && teamMembers.length > 0) || (teamMembers.every(user => currentWriters.includes(user.userEmail)))}
                        >
                            Next
                        </button>
                    </div>
