...
async function reassignQuestionsToSection(payload,req) {
    const { proposalID, originalSectionID, updatedSectionID } = payload;
    ...
    const questions = existingProposal.questions;
    ...
    req.log = {
        ...
        details: `questions from ${originalSectionID} has been reassigned to ${updatedSectionID} `,
    }
...
exports.reassignQuestions = async function(req, res, next) {
    if (req.method === "PUT") {
        try {
            const restoreResult = await reassignQuestionsToSection(req.body, req);
