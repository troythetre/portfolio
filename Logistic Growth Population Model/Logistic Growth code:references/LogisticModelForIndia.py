...
with open('India.csv') as csv_file:
...
populationModel = populationData*0.0
        ...
        while guessR < guessRHigh:
            ...
            while guessA < guessAHigh:
                ...
                while n < len(finePopulationModel)-1:
                    finePopulationModel[n+1] = finePopulationModel[n]+h*guessR*finePopulationModel[n]*(1-(finePopulationModel[n]+guessA*finePopulationModel[n]**2)/guessK)
                    n += 1
                n = 0
...
while n < len(populationModelExtendPositive)-1:
    populationModelExtendPositive[n+1] = populationModelExtendPositive[n]+h*hitR*populationModelExtendPositive[n]*(1-(populationModelExtendPositive[n]+hitA*populationModelExtendPositive[n]**2)/hitK)
    n += 1
...
while n < len(populationModelExtendNegative)-1:
    populationModelExtendNegative[n+1] = populationModelExtendNegative[n]-h*hitR*populationModelExtendNegative[n]*(1-(populationModelExtendNegative[n]+hitA*populationModelExtendNegative[n]**2)/hitK)
    n += 1

plt.subplot(121)
plt.suptitle("Logistic Model:", fontsize=14, fontweight='bold')
...
plt.plot(yearExtendNegative, populationModelExtendNegative, 'g', linewidth=0.5)
plt.grid()
plt.axis([-10,70,0.9,3.5])
plt.title('India Population (Zoom-In)')
plt.ylabel("Popultion (normalized to 1960's)")
plt.xlabel("Year (offset to 1960)")
plt.show()
